...
import {
  addCommentReplyToPostApi,
  addCommentToPostApi,
  deleteCommentToPostApi,
  removeCommentReplyToPostApi,
...
function CommentBox({ item }) {
  ...
  const { setCommentResponse } = useContext(commentResponseContext);
  ...
  const handleAdCommentReply = async () => {
    ...
    if (currentUser) {
      ...
      if (!commentReplyData.reply || !commentReplyData.username) {
        ...
        const res = await addCommentReplyToPostApi(
          commentReplyData,
          item._id,
          replyCommentId,
          reqHeader
  ...
  const handleDeleteComment = async (item, items) => {
    if (currentUser) {
      const res = await deleteCommentToPostApi(
        item._id,
        items.commentId,
        reqHeader
      );
  ...
  const handledeleteCommentReplay = async (item, items) => {
    if (currentUser) {
      const res = await removeCommentReplyToPostApi(
        item._id,
        items.commentId,
        reqHeader
      );
  ...
  return (
    <div>
      <button
        onClick={handleShow}
        className="border-0 bg-white p-0 d-flex flex-column"
      >
                      ...
                      <div className=" d-flex justify-content-between">
                        <div>
                          ...
                          <h6
                            ...
                            className="text-start mb-0"
                          >
                            {items?.dateOfCommented}
                          </h6>
                          <h6 className="text-start">{items.comment}</h6>
                        </div>
                        <div className="d-grid mb-2">
                          <button
                            className="border-1 px-1 bg-white mb-1"
                            onClick={() => handleChangeToReply(items.commentId)}
                          >
                            reply
                          </button>
                          {currentUser && items.userId === currentUser._id && (
                            <i
                              className="fa-solid fa-trash text-end"
                              style={{ cursor: "pointer" }}
                              onClick={() => handleDeleteComment(item, items)}
                            ></i>
                          )}
                        </div>
                      </div>
                                ...
                                <div className="d-flex justify-content-between w-100">
                                  {currentUser &&
                                    reply.userId === currentUser._id && (
                                      <i
                                        className="fa-solid fa-trash text-end my-auto"
                                        style={{ cursor: "pointer" }}
                                        onClick={() =>
                                          handledeleteCommentReplay(item, items)
                                        }
                                      ></i>
                                    )}
                                  <div>
                ...
                {replyBtn ? (
                  <div>
                    <h6
                      ...
                      onClick={handleChangeToComment}
                    >
                      cancel reply <i class="fa-solid fa-xmark"></i>
                    </h6>
                    <form
                      action=""
                      className="d-flex w-100"
                      onSubmit={handleAdCommentReply}
                    >
                      <input
                        type="text"
                      ...
                      <button
                        ...
                        disabled={!currentUser || !commentReplyData.reply}
                      >
                  ...
                  >
                    ...
                    <button
                      className="border-0 p-2 bg-white"
                      disabled={!currentUser || !commentData.comment}
                    >
                      <i class="fa-brands fa-telegram fa-xl"></i>
                    </button>
                  </form>

...
import { addLikeToPostAllApi, addSavedPostInAllPostApi, addSavedPostUserApi, addUserLikedPostAllApi, deletePostApi, deletePostInUserApi, getAllPostsApi, getCurrentUserApi, removeLikeToPostAllApi, removeSavedPostInAllPostApi, removeSavedPostUserApi, removeUserLikedPostAllApi } from '../Services/allApis'
        ...
        <div>
            ...
            <div className='p-3' style={{ backgroundColor: "#fff", borderRadius: "10px" }}>
                <Tabs defaultActiveKey="profile" id="fill-tab-example" className="mb-3" fill style={{ color: "#000" }}>
                    <Tab eventKey="profile" title={<i class="fa-solid fa-table"></i>} style={{ minHeight: "55vh" }}>
                        <Row className='d-flex p-3'>
                            {
                                myPosts.length > 0 ?
                                    myPosts.map(item => (
                                        <Col xs="4" style={{ border: "1px solid #f0eef6" }} className='p-2 '>

                                            <div className='d-flex' style={{ height: "150px" }}>
                                                <img src={`${BASE_URL}/upload/${item.image}`} alt="" className='img-fluid w-100' />
                                                <i className="fa-solid fa-trash" style={{ cursor: "pointer", position: "relative", right: "20px", top: "10px" }} onClick={() => handlePostDelete(item)}></i>
                                            </div>
                                        </Col>
                                    ))
                                    :
                    ...
                    <Tab eventKey="profile1" title={<i class="fa-solid fa-mobile-screen-button"></i>}>
                        {
                            myPosts.length > 0 ?
                                myPosts.map(item => (
                                    <Row className='g-0 mb-4 p-2' style={{ backgroundColor: "#fff", borderRadius: "10px" }}>
                                        <Col xs={1} className='d-flex justify-content-center align-items-center'>
                                            <img src={`${BASE_URL}/upload/${currentUser.image}`} alt="" style={{ width: "40px", height: "40px", borderRadius: "50%" }} />
                                        </Col>
                                        <Col xs={8} className='px-3'>
                                        ...
                                        </Col>
                                            {
                                                ...
                                                <Col xs={3} className=' d-flex justify-content-end align-items-center'>
                                                    ...
                                                    <i className="fa-solid fa-trash ms-3 " style={{ cursor: "pointer" }} onClick={() => handlePostDelete(item)}></i>
                                                </Col>
                                            }
